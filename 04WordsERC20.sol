...
contract WordsonChainERC20 {
    ...
    uint TokenTotalSupply = 1000000000; // 1B total supply
    ...
    function transfer(address _to, uint _amount) public returns (bool success) {
        ...
        require(balances[msg.sender] >= _amount); //체크
        
        balances[msg.sender] -= _amount; //차감

        balances[_to] += _amount; //전송
        
        emit Transfer(msg.sender, _to, _amount); //이벤트
    ...
    function approve(address _spender, uint _amount) public returns (bool success) {
        ...
        // mapping(address => mapping(address => uint)) public allowance; 구조임. 따라서 mapping을 이어서 위와 같이 사용.

        // ERC20의 기본 코드는 allowance[msg.sender][_spender] = _amount;
    ...
    function increaseAllowance(address _spender, uint _increaseamount) public returns (bool success) {
        ...
        uint currentAllowance = allowance[msg.sender][_spender];
        
        allowance[msg.sender][_spender] = currentAllowance - _increaseamount;

        emit Approval(msg.sender, _spender, currentAllowance);
    ...
    function decreaseAllowance(address _spender, uint _decreaseamount) public returns (bool success) {
        ...
        uint currentAllowance = allowance[msg.sender][_spender]; //매번 호출하지 말고 처음에 memory에 저장. -> 가스비 절약

        require(currentAllowance >= _decreaseamount, "allowance must be bigger than _decreaseamount");
        
        allowance[msg.sender][_spender] = currentAllowance - _decreaseamount;
    ...
    function transferFrom(address _from, address _to, uint _amount) public returns (bool success) {
        ...
        allowance[_from][msg.sender] = currentAllowance - _amount; // allowance 차감
